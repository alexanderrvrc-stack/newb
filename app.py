...
def highscores_page():
    ...
    if knight_scores:
        for i, score in enumerate(knight_scores, 1):
            ...
            wave = score.get('wave', 'N/A')
            knight_table += f"<tr><td>{medal}</td><td>{score['name']}</td><td>{score['score']:,}</td><td>Lv.{level}</td><td>Wave {wave}</td></tr>"
    else:
        knight_table = "<tr><td colspan='5'>No scores yet! Be the first knight to survive!</td></tr>"
    ...
    <head>
        ...
        <style>
            ...
            h2 {{ color: #666; border-bottom: 2px solid #eee; padding-bottom: 10px; margin-top: 40px; }}
            ...
            .nav-links a {{ margin: 0 15px; color: #007bff; text-decoration: none; font-weight: bold; padding: 10px 20px; background: #f8f9fa; border-radius: 5px; display: inline-block; }}
    ...
    <body>
        <div class="container">
            <h1>🏆 Hall of Fame - High Score Leaderboards</h1>
            
            <div class="game-section">
                <h2>⚔️ Knight vs Slimes Survival</h2>
                <table>
                    <thead>
                        <tr>
                            <th>Rank</th>
            ...
            <div class="nav-links">
                <a href="/">🏠 Home</a>
                <a href="/knight-game">⚔️ Play Knight Game</a>
